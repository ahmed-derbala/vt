...
let log = exports.log = (logFilename) => {
    ...
    if (prefs.emails.send == true || process.env.NODE_ENV == 'production' || process.env.NODE_ENV == 'testing') {
...
//non-production envirement
log.stream = {
    write: function (request, encoding) {
        let statusCode = request.slice(0, 1)
        if (statusCode == 2) {
            log().verbose(colors.green(request));
        }
        else if (statusCode == 3) {
            log().verbose(colors.cyan(request));
        }
        else if (statusCode == 4) {
            log().verbose(colors.yellow(request));
        } else {
            log().verbose(colors.red(request));
        }
        //checkin responsetimealert
         let rt = request.slice(request.indexOf('[*') + 2, request.indexOf('*]')) //rt like response time
